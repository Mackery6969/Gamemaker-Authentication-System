...
| `register_command.py` | Registers the `/generate`, `/generatefor`, `/cancel`, `/cancelrun`, `/cancel-build`, `/list`, `/queue`, `/dispatch`, `/history` slash commands in your tester guild. Re-run after changing `AVAILABLE_BRANCHES` / `INCLUDE_PR_BRANCHES`. |
...
the numeric Worker run id shown in `/queue`. `/cancel-build` does the same for
a tester build, matched on the build id shown in parentheses in `/queue`.
