...
export async function verifyDiscordSig(
  req: Request,
  body: string,
  env: Env,
): Promise<boolean> {
...
export async function isMember(env: Env, uid: string): Promise<boolean | null> {
  const r = await fetch(
    `${DISCORD}/guilds/${env.TESTER_GUILD_ID}/members/${uid}`,
    {
      headers: {
...
export async function dmUser(
  ...
  try {
    ...
    if (!ch.ok) {
      console.error("dm channel open failed", ch.status);
      return;
    }
...
export async function postToChannel(
  ...
  try {
    ...
    if (!r.ok)
      console.error("channel post failed", channelId, r.status, await r.text());
  } catch (e) {
...
export async function postEmbedToChannel(
  ...
  try {
    ...
    if (!r.ok)
      console.error("embed post failed", channelId, r.status, await r.text());
  } catch (e) {
...
export const EMBED_COLOR_CANCELED = 0x95a5a6;

export async function alertDenied(
  env: Env,
  buildId: string,
...
export async function notifyBuildReady(
  env: Env,
  testerId: string,
  token: string,
): Promise<void> {
...
export async function alertBuildSigBad(
  env: Env,
  buildId: string,
  reason: string,
): Promise<void> {
...
export async function alertMintFailed(
  env: Env,
  buildId: string,
  baseKey: string,
  reason: string,
...
export async function alertUpdateFailed(
  env: Env,
  buildId: string,
  user: { id: string; username?: string },
  reason: string,
