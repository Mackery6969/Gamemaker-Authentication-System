...
export async function callback(
  ...
  if (sess.status !== "pending")
    return page("This session was already used.", false, 400);
  if (!code) {
    await finalize(env, state, sess, null, null, null, "error");
    return page("Login was cancelled.", false, 400);
  }
  ...
  if (sess.kind === "download")
    return await downloadAuth(env, state, sess, user, ctx);
  if (sess.kind === "update")
    return await updateAuth(env, state, sess, user, ctx);
  ...
  else if (member && (guildOnly || dev || user.id === build.tester_id))
    outcome = "allow";
  else outcome = "deny";
  ...
  if (outcome === "allow")
    return page(
      `Verified as ${esc(user.username || user.id)}. Return to the game.`,
      true,
    );
  if (outcome === "error")
    return page(
      "Couldn't verify your membership right now — relaunch the game to try again.",
      false,
    );
  const reason = member
    ? "this build is not assigned to your account"
    : "you are not in the tester server";
...
async function finalize(
  ...
  sess.sig = await sign(
    env.SIGNING_SECRET,
    `${state}|${outcome}|${user?.id ?? ""}`,
  );
  if (outcome === "allow" && user)
    sess.device_token = await signDeviceToken(env, user.id);
  await env.SESSIONS.put(state, JSON.stringify(sess), {
    expirationTtl: ttl(env),
  });
...
export async function result(url: URL, env: Env): Promise<Response> {
  ...
  if (sess.kind === "update") {
    return json({
      status: "done",
      verdict: sess.authorized ? "allow" : "deny",
      mode: sess.update_mode,
      target_sha: sess.update_target_sha,
      package_sha256: sess.update_package_sha256,
      verify: sess.update_verify,
      download_url: sess.authorized
...
export async function registerBuild(req: Request, env: Env): Promise<Response> {
  ...
  if (!buildId || !testerId)
    return json({ error: "build_id and tester_id required" }, 400);
  const rec: Build = {
    tester_id: testerId,
    label: b.label || "",
    created: Date.now(),
