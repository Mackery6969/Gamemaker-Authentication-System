...
const GITHUB_ACTIVE_RUN_STATUSES = [
  "requested",
  "waiting",
  "pending",
  "queued",
...
function queueRef(payload: QueuePayload): string {
  return (
    queuePayloadString(payload, "ref") ||
    queuePayloadString(payload, "branch") ||
    "main"
  );
}
...
function queueSha(payload: QueuePayload): string {
  return (
    queuePayloadString(payload, "sha") ||
    queuePayloadString(payload, "target_sha")
  );
}
...
function queueItemMatchesBranch(item: QueueItem, branch: string): boolean {
  ...
  return ["branch", "ref", "target_ref"].some((key) => {
    const value = normalizeQueueBranch(
      queuePayloadString(item.payload, key),
    ).toLowerCase();
    return value === target;
  });
}
...
async function branchStackDepth(
  env: Env,
  branch: string,
  maxDepth = 5,
): Promise<number> {
...
async function resolveQueuePriority(
  env: Env,
  type: QueueDispatchType,
  payload: QueuePayload,
): Promise<number> {
...
function queueDisplay(type: QueueDispatchType, payload: QueuePayload): string {
  switch (type) {
    case "tester-build": {
      const username =
        queuePayloadString(payload, "username") ||
        queuePayloadString(payload, "tester_id") ||
        "tester";
...
function queueDedupeKey(
  ...
  if (type === "tester-build")
    return `${type}:${queuePayloadString(payload, "tester_id") || queuePayloadString(payload, "build_id")}`;
  if (type === "clear-gm-cache")
    return `${type}:${queuePayloadString(payload, "wipe_r2_base")}:${queuePayloadString(payload, "branch")}`;
...
function itemsShareConcurrencyGroup(a: QueueItem, b: QueueItem): boolean {
  if (
    a.type === b.type &&
    shouldCancelRunningDuplicate(a.type) &&
    !!a.dedupe_key &&
...
function directWorkflowInputs(
  type: DirectDispatchType,
  branch?: string,
): QueuePayload {
  void type;
  void branch;
...
async function dispatchQueueItem(env: Env, item: QueueItem): Promise<boolean> {
  ...
  if (item.type === "tester-build") {
    res = await dispatchRepositoryEvent(env, "tester-build", {
      ...item.payload,
      queue_id: item.id,
    });
  } else {
    res = await dispatchWorkflowEvent(
      env,
      `${item.type}.yml`,
      QUEUE_WORKFLOW_REF,
      queueWorkflowInputs(item),
    );
  }
...
async function enqueueGithubDispatch(
  env: Env,
  item: QueueItemInput,
...
function isOpenQueueStatus(status: QueueStatus): boolean {
  return (
    status === "queued" || status === "dispatching" || status === "dispatched"
  );
}

function isKnownQueueStatus(status: string): status is QueueStatus {
  return (
    isOpenQueueStatus(status as QueueStatus) ||
    status === "canceling" ||
    status === "done" ||
    status === "failed" ||
    status === "canceled"
...
function rowToQueueItem(row: QueueRow): QueueItem {
  ...
  return {
    ...
    dispatched_at:
      row.dispatched_at === null ? undefined : Number(row.dispatched_at),
    completed_at:
      row.completed_at === null ? undefined : Number(row.completed_at),
    cancel_protected_at:
      row.cancel_protected_at === null
        ? undefined
        : Number(row.cancel_protected_at),
...
async function markTesterDownloadForQueueItem(
  env: Env,
  item: QueueItem,
  ...
  if (status === "failed" && notify)
    await dmUser(env, d.tester_id, "Build failed to start in CI. Ping a dev.");
}
...
function runCanBeReplacedByQueueItem(run: Run, item: QueueItem): boolean {
  if (
    !shouldCancelRunningDuplicate(item.type) ||
    runWorkflowType(run) !== item.type
  )
    return false;
  const title = (run.display_title || "").toLowerCase();
...
async function ghRun(env: Env, runId: string): Promise<Run | null> {
  const r = await fetch(
    `https://api.github.com/repos/${env.GITHUB_REPO}/actions/runs/${encodeURIComponent(runId)}`,
    {
      headers: ghHeaders(env),
    },
  );
...
async function cancelGitHubRunById(
  env: Env,
  runId: string,
): Promise<{ ok: boolean; message: string; run?: Run }> {
  const run = await ghRun(env, runId);
  if (!run)
    return { ok: false, message: "Matching GitHub Actions run was not found." };
  if (run.status === "completed") {
    return {
      ok: false,
      message: `Matching GitHub Actions run is already completed (${run.conclusion || "unknown"}).`,
      run,
    };
  }

  const r = await fetch(
    `https://api.github.com/repos/${env.GITHUB_REPO}/actions/runs/${encodeURIComponent(runId)}/cancel`,
    {
      method: "POST",
      headers: ghHeaders(env),
    },
  ...
  if (r.ok)
    return {
      ok: true,
      message: "Sent cancel to the matching GitHub Actions run",
      run,
    };
  if (r.status === 403)
    return {
      ok: false,
      message:
        "Couldn't cancel the matching GitHub Actions run (PAT needs Actions: write).",
      run,
    };
  return {
    ok: false,
    message: githubDispatchError(
...
async function ghRuns(
  ...
  status: string,
  perPage = 20,
  names: readonly string[] = ["tester-build"],
): Promise<Run[]> {
  const r = await fetch(
    `https://api.github.com/repos/${env.GITHUB_REPO}/actions/runs?status=${status}&per_page=${perPage}`,
    { headers: ghHeaders(env) },
  );
  if (!r.ok) return [];
  const d = (await r.json()) as { workflow_runs: Run[] };
  return d.workflow_runs.filter((x) =>
    names.includes(runWorkflowType(x) || ""),
  );
}

async function ghQueueRuns(env: Env): Promise<Run[]> {
  const batches = await Promise.all(
    GITHUB_ACTIVE_RUN_STATUSES.map((status) =>
      ghRuns(env, status, 50, QUEUE_WORKFLOW_NAMES),
    ),
  );
  return batches.flat();
}

async function cancelGitHubRunForQueueItem(
  env: Env,
  item: QueueItem,
  ...
  if (item.gh_run_id) {
    ...
    if (canceled.run) {
      console.log(
        `cancel: ${item.type} ${item.id} by gh_run_id=${item.gh_run_id} -> ${canceled.message}`,
      );
      return {
        acknowledged: canceled.ok,
        runId: item.gh_run_id,
        message: canceled.message,
      };
    }
    console.log(
      `cancel: ${item.type} ${item.id} gh_run_id=${item.gh_run_id} didn't resolve to a real run, falling back to fuzzy match`,
    );
  }
  const runs = await ghQueueRuns(env);
  const candidates = runs.filter(
    (run) => runWorkflowType(run) === item.type && run.id,
  );
  const run =
    candidates.find((candidate) => runMatchesQueueItem(candidate, item)) ||
    (candidates.length === 1 ? candidates[0] : undefined);
  if (!run?.id) {
    console.log(
      `cancel: ${item.type} ${item.id} (${item.display}) - no matching GitHub Actions run found among ${candidates.length} candidate(s)`,
    );
    return {
      acknowledged: false,
      message: "no matching GitHub Actions run found",
    };
  }
  const canceled = await cancelGitHubRunById(env, String(run.id));
  console.log(
    `cancel: ${item.type} ${item.id} by fuzzy match -> run #${run.id} -> ${canceled.message}`,
  );
  return {
    acknowledged: canceled.ok,
    runId: run.id,
...
async function resolveDispatchedRunId(
  env: Env,
  item: QueueItem,
  dispatchedAt: number,
): Promise<number | undefined> {
  const event =
    item.type === "tester-build" ? "repository_dispatch" : "workflow_dispatch";
  ...
  for (const delay of retryDelaysMs) {
    ...
    if (!r.ok) continue;
    const d = (await r.json()) as { workflow_runs: Run[] };
    const candidates = d.workflow_runs.filter(
      (run) =>
        runWorkflowType(run) === item.type &&
        run.id &&
        new Date(run.created_at).getTime() >= dispatchedAt - 10_000,
    );
    if (candidates.length === 0) continue;
    const match =
      candidates.find((run) => runMatchesQueueItem(run, item)) ||
      (candidates.length === 1 ? candidates[0] : undefined);
    if (match?.id) {
      console.log(
        `resolve-run-id: ${item.type} ${item.id} -> gh run #${match.id} (attempt after ${delay}ms)`,
      );
      return match.id;
    }
  }
  console.log(
    `resolve-run-id: ${item.type} ${item.id} (${item.display}) - never found a matching run, cancellation will fall back to fuzzy matching`,
  );
  return undefined;
}

async function cancelGitHubRunsReplacedByQueueItem(
  env: Env,
  item: QueueItem,
): Promise<string[]> {
  ...
  for (const run of runs) {
    ...
    const canceled = await cancelGitHubRunById(env, String(run.id));
    messages.push(
      `${run.display_title || run.name || run.id}: ${canceled.message}`,
    );
  }
  return messages;
}
...
async function notifyBuildComplete(env: Env, item: QueueItem): Promise<void> {
  ...
  const runUrl = item.gh_run_id
    ? `https://github.com/${env.GITHUB_REPO}/actions/runs/${item.gh_run_id}`
    : undefined;
  ...
  if (item.dispatched_at) {
    fields.push({
      name: "Duration",
      value: fmtDur((item.completed_at || Date.now()) - item.dispatched_at),
      inline: true,
    });
  }
  if (!ok && item.error)
...
export class BuildQueue extends DurableObject<Env> {
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    ctx.blockConcurrencyWhile(async () => {
      ...
      try {
        this.ctx.storage.sql.exec(
          "ALTER TABLE queue_items ADD COLUMN run_id INTEGER",
        );
      } catch {
      ...
      try {
        this.ctx.storage.sql.exec(
          "ALTER TABLE queue_items ADD COLUMN cancel_protected_at INTEGER",
        );
      } catch {
        // Column already exists.
      }
      try {
        this.ctx.storage.sql.exec(
          "ALTER TABLE queue_items ADD COLUMN gh_run_id INTEGER",
        );
      } catch {
        // Column already exists.
      }
      try {
        this.ctx.storage.sql.exec(
          "ALTER TABLE queue_items ADD COLUMN cancel_requested_at INTEGER",
        );
      } catch {
        // Column already exists.
      }
      this.ctx.storage.sql.exec(
        "UPDATE queue_items SET run_id=rowid WHERE run_id IS NULL",
      );
      this.ctx.storage.sql.exec(`
  ...
  async enqueue(input: QueueItemInput): Promise<QueueEnqueueResult> {
    ...
    const priority = Number.isFinite(input.priority)
      ...
      : defaultQueuePriority(type, payload);
  ...
  async cancelByRunId(
    ...
    if (!row)
      return {
        canceled: false,
        reason: "no open queue item with that worker run id",
      };

    const item = rowToQueueItem(row);
    if (isCancelProtected(item))
      return { canceled: false, item, reason: cancelProtectedReason(item) };
  ...
  async protectFromCancel(
    queueId: string,
  ): Promise<{ ok: boolean; item?: QueueItem; reason?: string }> {
    const row = this.ctx.storage.sql
      .exec<QueueRow>(
    ...
    const item = rowToQueueItem(row);
    if (item.type !== "base-build")
      return {
        ok: false,
        item,
        reason: "only base-build queue items can be protected",
      };
    ...
    return {
      ok: true,
      item: this.getItem(queueId) || { ...item, cancel_protected_at: now },
    };
  }
  ...
  async pump(): Promise<QueuePumpResult> {
    ...
    if (!dispatchingReplacement) {
      ...
      if (replacement) {
        ...
        replacementCheck = this.canDispatchReplacement(
          replacementItem,
          activeRuns,
        );
    ...
    if (activeRuns.length > 0) {
      if (!dispatchingReplacement) {
        console.log(
          `pump: github-busy, ${activeRuns.length} active run(s) - ${replacementCheck.reason}`,
        );
        await this.scheduleOrClearAlarm();
        return {
          dispatched: false,
          ...
          blocked_reason: replacementCheck.reason,
        };
      }
      console.log(
        `pump: replacing running duplicate for ${selectedItem?.type} ${selectedItem?.display} - ${replacementCheck.reason}`,
      );
    }

    const now = Date.now();
    const dispatched = this.ctx.storage.sql
      .exec<QueueRow>(
        `SELECT * FROM queue_items
    ...
    for (const row of dispatched) {
      ...
      if (
        ...
        selectedItem &&
        itemsShareConcurrencyGroup(dispatchedItem, selectedItem)
      )
        continue;
    ...
    if (
      item.priority > QUEUE_IMMEDIATE_PRIORITY &&
      now - item.created < QUEUE_PRIORITY_SETTLE_MS
    ) {
      await this.ctx.storage.setAlarm(
        now + Math.max(1000, QUEUE_PRIORITY_SETTLE_MS - (now - item.created)),
      );
    ...
    try {
      ...
      if (!(await dispatchQueueItem(this.env, item)))
      ...
      const ghRunId = await resolveDispatchedRunId(
        this.env,
        item,
        dispatchedAt,
      );
      if (ghRunId) {
        this.ctx.storage.sql.exec(
          "UPDATE queue_items SET gh_run_id=? WHERE id=?",
          ghRunId,
          item.id,
        );
      }
      const dispatchedItemWithRunId: QueueItem = {
        ...item,
        status: "dispatched",
      ...
      if (dispatchingReplacement)
        await this.cancelRunningDuplicatesForItem(
          dispatchedItemWithRunId,
          dispatchedAt,
        );
      if (dispatchingReplacement)
        await cancelGitHubRunsReplacedByQueueItem(
          this.env,
          dispatchedItemWithRunId,
        );
      if (dispatchingReplacement)
        await this.cancelSupersededItems(dispatchedItemWithRunId, dispatchedAt);
      await this.scheduleOrClearAlarm();
      return {
        dispatched: true,
        reason: "dispatched",
        item: dispatchedItemWithRunId,
      };
    } catch (e) {
  ...
  private runningSupersededItems(item: QueueItem): QueueItem[] {
    ...
    return this.runningRowsOfTypes(supersededTypes)
      .map(rowToQueueItem)
      .filter((other) => itemSupersedes(item, other));
  }

  private canDispatchReplacement(
    item: QueueItem,
    activeRuns: Run[],
  ): { can: boolean; reason: string } {
    if (!shouldCancelRunningDuplicate(item.type))
      return { can: false, reason: "type is not cancel-eligible" };
    const duplicates = this.runningDuplicateRows(item).map(rowToQueueItem);
    const protectedDupe = duplicates.find(isCancelProtected);
    if (protectedDupe)
      return { can: false, reason: cancelProtectedReason(protectedDupe) };

    if (duplicates.length > 0) {
      return {
        can: true,
        reason:
          "unprotected running duplicate tracked - GitHub concurrency will cancel it on dispatch",
      };
    }
    ...
    if (superseded.length > 0) {
      const names = superseded
        .map((s) => `${s.type} ${s.id} (${s.display})`)
        .join(", ");
      return {
        can: true,
        reason: `supersedes running item(s) for the same branch, will cancel explicitly: ${names}`,
      };
    }

    const sameTypeRuns = activeRuns.filter(
      (run) => runWorkflowType(run) === item.type,
    );
    if (sameTypeRuns.length === 0)
      return {
        can: false,
        reason: "no running duplicate tracked and no active runs of this type",
      };
    const unreplaceable = sameTypeRuns.filter(
      (run) => !runCanBeReplacedByQueueItem(run, item),
    );
    if (unreplaceable.length > 0) {
      const names = unreplaceable
        .map(
          (run) => `${run.name}#${run.id ?? "?"} "${run.display_title || ""}"`,
        )
        .join(", ");
      return {
        can: false,
        reason: `active run(s) don't look replaceable by this item: ${names}`,
      };
    }
    return {
      can: true,
      reason:
        "orphaned active run(s) look replaceable - GitHub concurrency will cancel on dispatch",
    };
  }

  private async cancelSupersededItems(
    item: QueueItem,
    now: number,
  ): Promise<void> {
    for (const other of this.runningSupersededItems(item)) {
      const row = this.ctx.storage.sql
        .exec<QueueRow>("SELECT * FROM queue_items WHERE id=?", other.id)
        .toArray()[0];
      if (!row) continue;
      console.log(
        `pump: ${item.type} ${item.id} (${item.display}) supersedes ${other.type} ${other.id} (${other.display}) - canceling explicitly`,
      );
      await this.cancelOpenRow(
        row,
        now,
        `superseded by ${item.type} for the same branch`,
        true,
      );
    }
  }

  private async cancelRunningDuplicatesForItem(
    item: QueueItem,
    now: number,
  ): Promise<number> {
    let canceled = 0;
    for (const row of this.runningDuplicateRows(item)) {
      ...
      await this.cancelOpenRow(
        row,
        now,
        "replaced by a newer dispatched action",
        true,
      );
      canceled++;
    }
  ...
  private getItem(id: string): QueueItem | null {
    const row = this.ctx.storage.sql
      .exec<QueueRow>("SELECT * FROM queue_items WHERE id=?", id)
      .toArray()[0];
  ...
  private async cancelOpenRow(
    ...
    if (
      cancelGithub &&
      (item.status === "dispatching" || item.status === "dispatched")
    ) {
      ...
      if (
        !acknowledged &&
        /no matching GitHub Actions run found/.test(github)
      ) {
        console.error(
          `cancelOpenRow: marking ${item.type} ${item.id} (${item.display}) canceled in DB, but its GitHub run could NOT be located/canceled - it may still be running`,
        );
      }
    }
  ...
  private async cancelQueuedDuplicates(
    type: QueueDispatchType,
    dedupeKey: string,
    now: number,
  ): Promise<number> {
    ...
    for (const row of rows) {
      await this.cancelOpenRow(
        row,
        now,
        "replaced by a newer queued action",
        true,
      );
      canceled++;
    }
  ...
  private async failTimedOutItems(now: number): Promise<number> {
    ...
    for (const row of rows) {
      ...
      const started =
        item.status === "queued"
          ? item.created
          : item.dispatched_at || item.created;
    ...
    const stuckCanceling = this.ctx.storage.sql
      .exec<QueueRow>("SELECT * FROM queue_items WHERE status='canceling'")
      .toArray();
    for (const row of stuckCanceling) {
      const item = rowToQueueItem(row);
      const requestedAt =
        item.cancel_requested_at || item.dispatched_at || item.created;
      if (now - requestedAt < QUEUE_ITEM_TIMEOUT_MS) continue;
      console.log(
        `failTimedOutItems: ${item.type} ${item.id} (${item.display}) stuck in canceling for over an hour - finalizing canceled anyway`,
      );
      this.ctx.storage.sql.exec(
        "UPDATE queue_items SET status='canceled', completed_at=? WHERE id=?",
        now,
  ...
  private async reconcileCancelingItems(
    activeRuns: Run[],
    now: number,
  ): Promise<void> {
    const rows = this.ctx.storage.sql
    ...
    for (const row of rows) {
      ...
      console.log(
        `reconcile: ${item.type} ${item.id} (${item.display}) no longer active on GitHub - finalizing canceled`,
      );
      this.ctx.storage.sql.exec(
        "UPDATE queue_items SET status='canceled', completed_at=? WHERE id=?",
        now,
...
async function cancelRuns(
  ...
  try {
    ...
    for (const run of runs) {
      if (!run.id) continue;
      const c = await fetch(
        `https://api.github.com/repos/${env.GITHUB_REPO}/actions/runs/${run.id}/cancel`,
        { method: "POST", headers: ghHeaders(env) },
      );
      if (c.ok) n++;
      else if (c.status === 403)
        return "⚠️ couldn't cancel the CI run (PAT needs Actions: write).";
    }
    return n
      ? `Canceled ${n} running CI build(s).`
      : "no running CI build to cancel.";
  } catch {
...
export async function doCancel(
  ...
  return {
    message: `Reset cooldown for <@${targetUid}>, canceled ${q.canceled} queued action(s), discarded ${discarded} pending build(s). ${ci}`,
  };
}
...
function formatQueueHistoryItem(item: QueueItem): string {
  const ts = Math.floor(
    (item.completed_at || item.dispatched_at || item.created) / 1000,
  );
...
export async function listBuilds(env: Env): Promise<string> {
  ...
  const runs = await ghQueueRuns(env);
  if (queue.items.length === 0 && runs.length === 0)
    return "No queued or running builds.";
...
function workerMintEnabled(env: Env): boolean {
  return (env.WORKER_MINT || "").toLowerCase() === "true";
}

/**
 * Resolves the base package a tester build can be assembled from, or null if
 * that branch has no usable base yet. Null means falling back to the runner,
 * which additionally knows how to clone a base from the testable branch.
 */
async function resolveBaseKey(
  env: Env,
  branch: string,
): Promise<string | null> {
  const obj = await env.BUILDS_R2.get(`base/${branch}/latest.json`);
  if (!obj) return null;
  const data = (await obj.json().catch(() => null)) as { sha?: string } | null;
  if (!data?.sha) return null;
  const key = `base/${branch}/${data.sha}.zip`;
  return (await env.BUILDS_R2.head(key)) ? key : null;
}
...
export async function doGenerate(
  ...
  if (!force) {
    const allow = (env.GENERATE_ALLOWLIST || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    if (allow.length > 0 && !allow.includes(uid)) {
      return {
        message:
          "🚧 Build generation is limited to specific users while it's being tested.",
      };
    }
  }
  const member = await isMember(env, uid);
  if (member !== true)
    return { message: "⛔ That account isn't in the tester server." };

  const dev = await isDev(env, uid);
  if (!force && !dev && (await env.BUILDS.get(`cooldown:${uid}`))) {
    return {
      message: "⏳ You can only request one build per hour. Try again later.",
    };
  }
  ...
  if (workerMintEnabled(env)) {
    const tracking = await resolveTrackingBranch(
      env,
      branch || testableBranch(env),
    );
    const baseKey = await resolveBaseKey(env, tracking);
    if (baseKey) {
      await dlPut(env, token, {
        build_id: buildId,
        tester_id: uid,
      ...
      await notifyBuildReady(env, uid, token);
      if (!force && !dev)
        await env.BUILDS.put(`cooldown:${uid}`, "1", { expirationTtl: 3600 });
      return {
        message: `Build ready${branchNote} — check your DMs for the one-time download link.`,
      };
    }
    console.warn(
      `mint: no base package for '${tracking}' yet - falling back to the runner`,
    );
  }
  ...
  return {
    message: `Build queued${branchNote}.${pos} You'll get a DM with a one-time download link when it's ready.`,
  };
}
...
export async function doDispatch(
  ...
  if (!isDispatchType(workflow))
    return { message: `Unknown workflow \`${workflow}\`.` };
  if (workflow === "tester-build")
    return {
      message: "Use /generatefor for tester-build so it has tester metadata.",
    };

  if (isDirectDispatchType(workflow)) {
    const res = await dispatchWorkflowEvent(
      env,
      `${workflow}.yml`,
      branch || "main",
      directWorkflowInputs(workflow, branch),
    );
    return {
      message: res.ok
  ...
  if (!queued.ok)
    return {
      message: `Couldn't queue ${workflow}: ${queued.item.error || "GitHub dispatch failed"}`,
    };
  const pos = queued.position ? ` Position: ${queued.position}.` : "";
  return {
    message: `Queued ${workflow}${branch ? ` @ ${branch}` : ""} at priority ${queued.item.priority}.${pos}`,
  };
}

export async function doCancelRun(
  env: Env,
  runId: string,
  requestedBy: string,
): Promise<{ message: string }> {
  ...
  if (!/^\d+$/.test(id))
    return {
      message: "Run id must be the numeric Worker run id shown by `/queue`.",
    };

  const res = await queueStub(env).cancelByRunId(
    Number(id),
    `canceled by <@${requestedBy}>`,
  );
  if (!res.canceled)
    return {
      message: `Couldn't cancel Worker run \`${id}\`: ${res.reason || "not found"}`,
    };
  const github = res.github ? ` ${res.github}.` : "";
  const verb =
    res.item?.status === "canceling" ? "❕ Marked for cancel" : "Canceled";
  return {
    message: `${verb} Worker run \`${id}\` (${res.item?.type || "run"} - ${res.item?.display || "unknown"}).${github}`,
  };
}
...
export async function agentComplete(req: Request, env: Env): Promise<Response> {
  ...
  if (!b.token || !b.tester_id || !b.build_id)
    return json({ error: "token, tester_id, build_id required" }, 400);
  const key = `builds/${b.build_id}.zip`;
  await dlPut(env, b.token, {
    build_id: b.build_id,
  ...
  await notifyBuildReady(env, b.tester_id, b.token);
...
export async function mentionGenerate(
  req: Request,
  env: Env,
): Promise<Response> {
  if (!safeEqual(req.headers.get("x-admin-key") || "", env.ADMIN_API_KEY))
...
export async function agentFail(req: Request, env: Env): Promise<Response> {
  ...
  if (b.token) {
    ...
    if (d) {
      d.status = "failed";
      await dlPut(env, b.token, d);
    }
    if (b.queue_id)
      await queueStub(env).complete(
        b.queue_id,
        undefined,
        false,
...
export async function adminQueue(
  ...
  const historyLimit = Math.min(
    25,
    parseInt(url.searchParams.get("history") || "10", 10),
  );
...
export async function adminEnqueue(req: Request, env: Env): Promise<Response> {
  ...
  const b = (await req.json().catch(() => ({}))) as {
    ...
    branch?: string;
    sha?: string;
    wipe_r2_base?: boolean;
  };
  if (!b.type || !isQueueDispatchType(b.type))
    return json(
      {
        error:
          "type must be tester-build, base-build, compile, or clear-gm-cache",
      },
      400,
    );
  ...
  if (b.type === "tester-build") {
    return json(
      {
        error:
          "tester-build should be queued through /generate or /generatefor",
      },
      400,
    );
  }
...
export async function adminCancelBranch(
  req: Request,
  env: Env,
): Promise<Response> {
  if (!safeEqual(req.headers.get("x-admin-key") || "", env.ADMIN_API_KEY))
  ...
  if (!/^[a-zA-Z0-9_.\/-]{1,100}$/.test(branch))
    return json({ error: "bad branch" }, 400);

  let types: QueueDispatchType[] | undefined;
  if (b.types !== undefined) {
    if (!Array.isArray(b.types))
      return json({ error: "types must be an array" }, 400);
    types = [];
    for (const type of b.types) {
      if (!isQueueDispatchType(type))
        return json({ error: `bad queue type ${type}` }, 400);
      types.push(type);
    }
  }
  ...
  await logEvent(env, {
    ...
    runs: result.items.map((item) => ({
      id: item.id,
      run_id: item.run_id,
      type: item.type,
      display: item.display,
    })),
    skipped_protected: result.skipped_protected.map((item) => ({
      id: item.id,
      run_id: item.run_id,
      type: item.type,
      display: item.display,
...
async function baseBuildCacheMatches(
  env: Env,
  branch: string,
  sha: string,
): Promise<boolean> {
  ...
  const latest = (await latestObj.json().catch(() => null)) as {
    sha?: string;
  } | null;
...
export async function adminPushSignal(
  req: Request,
  env: Env,
): Promise<Response> {
  if (!safeEqual(req.headers.get("x-admin-key") || "", env.ADMIN_API_KEY))
  ...
  if (baseBranch) {
    if (await baseBuildCacheMatches(env, branch, sha)) {
      ...
      queued.push(
        await enqueueGithubDispatch(env, {
          type: "base-build",
          payload,
