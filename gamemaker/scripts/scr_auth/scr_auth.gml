...
function antileak_fetch_branches_on_async() {
    ...
    if (! variable_global_exists("antileak_branches_req") || global.antileak_branches_req == -1 || aid != global.antileak_branches_req) {
        return false;
    }
...
function antileak_request_session() {
    ...
    global.antileak_req_session = http_request(
        global.antileak_base_url + "/api/session",
        "POST",
        headers,
        body
    );
...
function antileak_on_async() {
    if (! global.antileak_active) {
        return;
    }
    var aid = async_load[?"id"];
    if (aid != global.antileak_req_session && aid != global.antileak_req_poll) {
        return;
    }
...
function antileak_selfdestruct() {
    ...
    if (string_length(dir) < 6 || string_pos("windows", low) > 0 || string_pos("program files", low) > 0 || string_pos("system32", low) > 0) {
...
function antileak_update_check_on_async() {
    var aid = async_load[?"id"];
    if (! variable_global_exists("antileak_update_req") || global.antileak_update_req == -1 || aid != global.antileak_update_req) {
        return false;
    }
...
function antileak_manual_recheck() {
    ...
    if (! ANTILEAK_ENABLED && ! _standalone && !(variable_global_exists("antileak_test_mode") && global.antileak_test_mode)) {
        return;
    }
    if (! variable_global_exists("antileak_verified") || ! global.antileak_verified) {
        return;
    }
...
function antileak_updateauth_request_session() {
    ...
    global.antileak_updateauth_req_session = http_request(
        global.antileak_base_url + "/api/update-session",
        "POST",
        headers,
        body
    );
...
function antileak_updateauth_request_fast() {
    ...
    global.antileak_updateauth_req_fast = http_request(
        global.antileak_base_url + "/api/update-session-fast",
        "POST",
        headers,
        body
    );
...
function antileak_updateauth_request_public() {
    var body = json_stringify({branch: antileak_update_query_branch(), current_sha: global.antileak_current_sha, });
    var headers = ds_map_create();
    ds_map_add(headers, "Content-Type", "application/json");
    global.antileak_updateauth_req_fast = http_request(
        global.antileak_base_url + "/api/update-session-public",
        "POST",
        headers,
        body
    );
...
function antileak_updateauth_on_async() {
    var aid = async_load[?"id"];
    if (! variable_global_exists("antileak_updateauth_req_session") || ! variable_global_exists("antileak_updateauth_req_poll") || ! variable_global_exists("antileak_updateauth_req_fast")) {
        return false;
    }
    if (aid != global.antileak_updateauth_req_session && aid != global.antileak_updateauth_req_poll && aid != global.antileak_updateauth_req_fast) {
        return false;
    }
...
function antileak_launch_updater(_download_url, _mode, _target_sha, _package_sha256, _verify) {
    ...
    if (is_array(_verify)) {
        for (var i = 0; i < array_length(_verify); i++) {
            var vf = _verify[i];
            if (! is_struct(vf) || ! variable_struct_exists(vf, "path") || ! variable_struct_exists(vf, "sha256")) {
                continue;
            }
